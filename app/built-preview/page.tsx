...
function playIntroSound(ctx: AudioContext) {
  ...
  // instant tap feedback — soft blip
  tone(ctx, now, 660, 0.10, 0.26, 'triangle')
  // boot swell — warm pad in an audible register
  tone(ctx, now + 0.04, 247, 1.9, 0.20, 'sine')          // B3
  tone(ctx, now + 0.04, 330, 1.7, 0.10, 'sine', 392)     // E4 → G4 lift
  // ship chime — gentle two notes on the "shipped" beat (~3.4s)
  tone(ctx, now + 3.40, 587.33, 0.7, 0.24, 'triangle')   // D5
  tone(ctx, now + 3.62, 880.00, 1.0, 0.28, 'triangle')   // A5
}

/* ─────────────────────────────────────────────
   "BUILT LIVE" HERO — standalone preview
   Concept 03: the hero assembles itself from a
   wireframe skeleton into the shipped layout.
   from: "you code your own work"
───────────────────────────────────────────── */

/* subtle single tone — warm off-white skeleton blocks */
const BOX_COLORS = ['#F3F0E9', '#F3F0E9', '#F3F0E9']
...
export default function BuiltPreview() {
  const rootRef = useRef<HTMLDivElement>(null)
  const tlRef = useRef<gsap.core.Timeline | null>(null)

  function run() {
    // kill any in-flight timeline so replay / StrictMode don't fight
    if (tlRef.current) tlRef.current.kill()
    const q = gsap.utils.selector(rootRef)
    const tl = gsap.timeline()
    tlRef.current = tl

    // force the scenery video to play
    ...
    gsap.set(q('.wf-box'), { opacity: 0, scale: 0.96, '--dash': 1 })
    // clear any shimmer left from a previous run
    ...
    tl
      // 0. scenery video drifts in faintly under the blueprint (Ken Burns)
      ...
      // 3. solidify — dashed wireframe → shimmering skeleton (loading)
      ...
      // background color bloom warms the canvas
      .to(q('.color-bloom'), { opacity: 1, duration: 1.2, ease: 'power2.out' }, 1.7)

      // 4. blocks resolve into real content
      // eyebrow block → text
      ...
      // name block wipes away revealing serif name
      ...
      // 5. SHIP — blueprint fades, scenery comes alive, shipped stamp
  ...
  useEffect(() => {
    // scenery video plays muted behind the gate (no play glyph)
    ...
    // pre-set the hidden state so nothing flashes behind the gate
    const q = gsap.utils.selector(rootRef)
    gsap.set(q('.name-cover'), { scaleX: 1 })
  ...
  // the one tap that unlocks audio + fires the synced experience
  const enter = () => {
    if (entered) return
    try {
  ...
  const replay = () => {
    ...
    if (ctx) { if (ctx.state === 'suspended') ctx.resume(); playIntroSound(ctx) }
    run()
  }
  ...
  return (
    <div ref={rootRef} style={{
      ...
      fontFamily: "'Satoshi', sans-serif", zIndex: 9500,
    }}>
      {/* skeleton-loading shimmer — like an app still connecting */}
      <style>{`
        .sk-shimmer {
          background: linear-gradient(100deg, #EFEBE1 25%, #FBFAF6 50%, #EFEBE1 75%);
      ...
      {/* Scenery video — washed bright, drifts in (Ken Burns) */}
      <video className="scenery-video" muted playsInline autoPlay loop
        src="https://d8j0ntlcm91z4.cloudfront.net/user_38xzZboKViGWJOttwIXH07lWA1P/hf_20260328_083109_283f3553-e28f-428b-a723-d639c617eb2b.mp4"
        style={{
          position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover',
          zIndex: 0, opacity: 0, filter: 'brightness(1.25) saturate(0.78) contrast(0.96)',
        }} />
      {/* Bright wash overlay keeps it light & airy */}
      ...
      {/* Dot grid canvas — faint neutral */}
      <div className="bl-grid" style={{
        position: 'absolute', inset: 0, zIndex: 1, pointerEvents: 'none',
        backgroundImage: 'radial-gradient(rgba(13,13,13,0.10) 1px, transparent 1px)',
        backgroundSize: '26px 26px',
      }} />

      {/* Soft warm glow on ship — barely-there */}
      ...
      {/* Build log — top left, monospace */}
      <div style={{
        position: 'absolute', top: 'clamp(40px, 7vh, 90px)', left: 'clamp(32px, 6vw, 96px)',
        zIndex: 5, display: 'flex', flexDirection: 'column', gap: 5,
        fontFamily: "ui-monospace, 'SF Mono', Menlo, monospace",
        fontSize: 11, color: 'rgba(13,13,13,0.4)', letterSpacing: '0.02em',
      }}>
      ...
      {/* Tags — top right (wireframe boxes resolve to these) */}
      {/* Right rail — auto-rotating list of everything she brings */}
      <div className="tag-rail" style={{
        position: 'absolute', right: 'clamp(32px, 6vw, 96px)', top: 'clamp(56px, 9vh, 120px)', zIndex: 4,
      }}>
        <RotatingStrengths />
      </div>

      {/* Bottom-left block — wireframe + real stacked */}
      <div style={{
        position: 'absolute', left: 'clamp(32px, 6vw, 96px)', bottom: 'clamp(56px, 10vh, 120px)', zIndex: 4,
      }}>
        {/* EYEBROW */}
        <div style={{ position: 'relative', height: 16, marginBottom: 14 }}>
          ...
          <div className="real-eyebrow" style={{
            ...
            fontFamily: "'Cabinet Grotesk', sans-serif", fontSize: 10, fontWeight: 500,
            letterSpacing: '0.26em', color: 'rgba(13,13,13,0.45)',
          }}>PORTFOLIO · 2025</div>
        </div>
        ...
        <div style={{ position: 'relative', marginBottom: 16 }}>
          ...
          <div style={{ position: 'absolute', top: 0, left: 0, overflow: 'hidden' }}>
            ...
            <div className="name-cover sk-shimmer" style={{
              position: 'absolute', inset: 0, transformOrigin: 'right center',
            }} />
          </div>
        </div>
        ...
        <div style={{ position: 'relative', height: 22 }}>
          ...
          <div className="real-sub" style={{
            position: 'absolute', top: 0, left: 0,
            fontFamily: "'Satoshi', sans-serif", fontSize: 'clamp(13px, 1.3vw, 17px)',
            fontWeight: 400, fontStyle: 'italic', color: 'rgba(13,13,13,0.5)',
          }}>Product designer</div>
        </div>

        {/* dimension tick under name */}
        <div className="wf-dim" style={{
          position: 'absolute', left: 0, bottom: -2, width: 560, height: 1,
          background: 'repeating-linear-gradient(90deg, rgba(13,13,13,0.3) 0 4px, transparent 4px 8px)',
          transformOrigin: 'left center',
        }} />
      </div>

      {/* shipped stamp — bottom right */}
      <div className="shipped" style={{
        ...
        color: '#0d0d0d', border: '1px solid rgba(13,13,13,0.2)', borderRadius: 999,
      ...
      {entered && (
        <button onClick={replay} style={{
          ...
          fontFamily: "'Satoshi', sans-serif", fontSize: 11, fontWeight: 500, letterSpacing: '0.1em',
          textTransform: 'uppercase', color: 'rgba(13,13,13,0.4)', background: 'none',
          border: '1px solid rgba(13,13,13,0.15)', borderRadius: 999, padding: '8px 18px', cursor: 'pointer',
        }}>↻ Replay (with sound)</button>
      )}

      {/* ENTER GATE — the one tap that unlocks sound + fires the synced experience */}
      {!entered && (
        <div onClick={enter} className="enter-gate" style={{
          ...
          <div style={{
            ...
            letterSpacing: '0.2em', textTransform: 'uppercase', color: 'rgba(13,13,13,0.45)',
          }}>
...
/* wireframe box style helper — tinted dashed border per brand color */
function wfStyle(w: number, h: number, color = 'rgba(13,13,13,0.3)'): React.CSSProperties {
  return {
    width: w, height: h, maxWidth: '70vw',
    border: `1.5px dashed ${color}`, borderRadius: 4,
