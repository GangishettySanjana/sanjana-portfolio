...
export const ALL_VARIATIONS = [
  'charFill', 'charInverse', 'charAccent',
  'charAccentInverse', 'charAccentFill', 'charBorder',
] as const;
...
export class Typer {
  ...
  private stopLoop() {
    if (this.delayTimer) { window.clearTimeout(this.delayTimer); this.delayTimer = null; }
  ...
  private applyFrame() {
    ...
    const phase =
      this.type === 'inout' && this.frame > this.frames ? 'out' :
      this.type === 'inout' ? 'in' : this.type;
    const progress =
