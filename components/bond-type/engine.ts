...
export class BondType {
  ...
  private offsetAt(l: Letter, i: number, t: number): [number, number] {
    ...
    if (t < scatterEnd) {
      ...
      const p = sample(
        EASE_MOVE,
        ((local / (1 + spread)) * (EASE_MOVE.length - 1)) / MOVE_TICKS,
      );
      const from =
        k === 0
          ? [0, 0]
          : [l.px[this.seq[k - 1]], l.py[this.seq[k - 1]]];
  ...
  private jitter(i: number, amount: number): [number, number] {
    if (JITTER_CELLS <= 0 || amount <= 0 || amount < 0.5) return [0, 0];
  ...
  private render(t: number) {
    ...
    for (const [ia, ib] of this.pairs) {
      ...
      const bow =
        ...
        cell *
        Math.sin(
          this.clock * ((Math.PI * 2) / BOND_BOW_S) + ia * 1.1,
        );
      const nx = -uy;
      const ny = ux;
      ...
      for (let k = 0; k < n; k++) {
        ...
        const wt = BOND_WEIGHT_CELLS * cell;
        ctx.fillRect(
